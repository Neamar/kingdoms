...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on '_PendingMissionVariable', fields ['mission', 'name']
        db.delete_unique('mission_pendingmissionvariable', ['mission_id', 'name'])

        # Deleting field '_PendingMissionVariable.mission'
        db.delete_column('mission_pendingmissionvariable', 'mission_id')

        # Adding field '_PendingMissionVariable.pending_mission'
        db.add_column('mission_pendingmissionvariable', 'pending_mission',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['mission.PendingMission']),
                      keep_default=False)

        # Adding unique constraint on '_PendingMissionVariable', fields ['pending_mission', 'name']
        db.create_unique('mission_pendingmissionvariable', ['pending_mission_id', 'name'])


    def backwards(self, orm):
        # Removing unique constraint on '_PendingMissionVariable', fields ['pending_mission', 'name']
        db.delete_unique('mission_pendingmissionvariable', ['pending_mission_id', 'name'])


        # User chose to not deal with backwards NULL issues for '_PendingMissionVariable.mission'
        raise RuntimeError("Cannot reverse this migration. '_PendingMissionVariable.mission' and its values cannot be restored.")
        # Deleting field '_PendingMissionVariable.pending_mission'
        db.delete_column('mission_pendingmissionvariable', 'pending_mission_id')

        # Adding unique constraint on '_PendingMissionVariable', fields ['mission', 'name']
        db.create_unique('mission_pendingmissionvariable', ['mission_id', 'name'])
